...
# ########################################################################
# language translated message text: $language{$land}{'CUSTOMER'}  (=Customer)
# You can simply add here your translation...
# ########################################################################
my %language = ('en' => { "CUSTOMER" => 'Customer',
                          "NOTITYPE" => 'Notification Type',
                          "SNAME" => 'Service Name',
                          "SSTATUS" => 'Service Status',
                          "SGROUP" => 'Service Group',
                          "SDATA" => 'Service Data',
                          "HSTATUS" => 'Host Status',
                          "HNAME" => 'Hostname',
                          "HALIAS" => 'Hostalias',
                          ...
                          "AUTHOR" => 'Author',
                          "COMMENT" => 'Comment',
                          "HEADER" => 'Nagios Monitoring System Notification',
                          "FOOTER" => 'Generated by Nagios, the OpenSource monitoring solution' },
                'fr' => { "CUSTOMER" => 'Utilisateur',
                          "NOTITYPE" => 'Type de notification',
                          "SNAME" => 'Nom du service',
                          "SSTATUS" => 'Statut du service',
                          "SGROUP" => 'Groupe du service',
                          "SDATA" => 'Données',
                          "HSTATUS" => 'Statut d\'hôte',
                          "HNAME" => 'Nom d\'hôte',
                          "HALIAS" => 'Alias d\'hôte',
                          "HIP" => 'Adresse IP',
                          "HGROUP" => 'Groupe d\'hôte',
                          "ETIME" => 'Heure de la notification',
                          "HOUTPUT" => 'Données',
                          "AUTHOR" => 'Auteur',
                          "COMMENT" => 'Commentaire',
                          "HEADER" => 'Notification de la surveillance Nagios',
                          "FOOTER" => 'Généré par Nagios, le système de surveillance OpenSource' },
                'de' => { "CUSTOMER" => 'Anwender',
                          "NOTITYPE" => 'Nachrichtentyp',
                          "SNAME" => 'Dienstname',
                          "SSTATUS" => 'Dienstzustand',
                          "SGROUP" => 'Dienstgruppe',
                          "SDATA" => 'Dienstinformationen',
                          "HSTATUS" => 'Systemzustand',
                          "HNAME" => 'Systemname',
                          "HALIAS" => 'Systemalias',
                          "HIP" => 'System IP Adresse',
                          "HGROUP" => 'Systemgruppe',
                          "ETIME" => 'Meldungsdatum',
                          "HOUTPUT" => 'Systemnachricht',
                          "AUTHOR" => 'Author',
                          "COMMENT" => 'Kommentar',
                          "HEADER" => 'Nagios Überwachungssytem Meldung',
                          "FOOTER" => 'Erstellt mit Nagios, dem OpenSource Überwachungssytem' },
                'jp' => { "CUSTOMER" => '顧客名',
                          "NOTITYPE" => '通知の種類',
                          "SNAME" => 'サービス名',
                          "SSTATUS" => 'サービスの状態',
                          ...
                          "AUTHOR" => '投稿者',
                          "COMMENT" => 'コメント',
                          "HEADER" => 'Nagios 監視システムの通知',
                          "FOOTER" => 'このメッセージはオープンソースの監視システムNagiosで生成されています。' });
...
sub check_options {
 ...
 if (! defined($o_language)) {
  ...
  if (! $language{$land}{"CUSTOMER"}) { $land = $o_lang_def; }
}
...
sub create_message_text {
  $text_msg = $language{$land}{"HEADER"}."\n"
            . "=====================================\n\n";

  # if customer name was given for service providers, display it here
  if ( defined($o_customer)) {
    $text_msg .= $language{$land}{"CUSTOMER"} . ": $o_customer\n";
  }

  $text_msg .=$language{$land}{"NOTITYPE"} . ": $o_notificationtype\n"
            . $language{$land}{"SNAME"} . ": $o_servicedesc\n"
  ...
  if ( ( defined($o_notificationauth) && defined($o_notificationcmt) ) &&
    ...
    $text_msg .= $language{$land}{"AUTHOR"} . ": $o_notificationauth\n"
              .  $language{$land}{"COMMENT"} . ": $o_notificationcmt\n\n";

  }

  $text_msg .= "-------------------------------------\n"
            . $language{$land}{"FOOTER"} . "\n";
}
...
sub create_message_html {
  ...
  if ($o_format eq "multi" || $o_format eq "graph") {
    ...
    $html_msg .= "<th colspan=2><span>" . $language{$land}{"HEADER"} . "</span></th></tr><tr>\n"; }

  if ( defined($o_customer)) {
    $html_msg .= "<th colspan=2 class=customer>$o_customer</th></tr><tr>\n"; }

  $html_msg .= "<th width=$header_size class=even>" . $language{$land}{"NOTITYPE"} . ":</th>\n"
            . "<td bgcolor=$NOTIFICATIONCOLOR{$o_notificationtype}>\n"
            . "$o_notificationtype</td></tr>\n"
            . "<tr><th class=odd>" . $language{$land}{"SNAME"} . ":</th><td>$o_servicedesc</td></tr>\n"
            . "<tr><th class=even>" . $language{$land}{"SGROUP"} . ":</th><td class=even>\n";
  ...
  $html_msg .= "</td></tr>\n"
            . "<tr><th class=odd>" . $language{$land}{"SSTATUS"} . ":</th><td>$o_servicestate</td></tr>\n"
            . "<tr><th class=even>" . $language{$land}{"SDATA"} . ":</th><td class=even>\n";
  ...
  $html_msg .= "</td></tr>\n"
            .  "<tr><th class=odd>" . $language{$land}{"HNAME"} . ":</th><td>\n";
  ...
  $html_msg .= "</td></tr>\n"
            . "<tr><th class=even>" . $language{$land}{"HALIAS"} . ":</th><td class=even>$o_hostalias</td></tr>\n"
            . "<tr><th class=odd>" . $language{$land}{"HIP"} . ":</th><td>$o_hostaddress</td></tr>\n"
            . "<tr><th class=even>" . $language{$land}{"HGROUP"} . ":</th><td class=even>\n";
  ...
  $html_msg .= "</td></tr>\n"
            .  "<tr><th class=odd>" . $language{$land}{"ETIME"} . ":</th><td>$o_datetime</td></tr>\n";

  # If the author and comment data has been passed from nagios
  # and these variables have content, then we add two more columns
  if ( ( defined($o_notificationauth) && defined($o_notificationcmt) ) &&
       ( ($o_notificationauth ne "") && ($o_notificationcmt ne "") ) ) {
    $html_msg .= "<tr><th class=even>" . $language{$land}{"AUTHOR"} . ":</th>\n"
              . "<td class=even>$o_notificationauth</td></tr>\n"
              . "<tr><th class=odd>" . $language{$land}{"COMMENT"} . ":</th>\n"
